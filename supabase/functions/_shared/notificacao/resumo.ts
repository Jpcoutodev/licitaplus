/**
 * Geração do resumo com IA — provedor MiniMax (endpoint compatível com o
 * formato OpenAI de chat completions). Único ponto do sistema que fala com a
 * API de IA; trocar de provedor = trocar este arquivo.
 *
...
export async function gerarResumo(
  ...
  const resposta = await fetchWithRetry(`${baseUrl}/chat/completions`, {
    method: "POST",
    headers: {
 ...
 * conteúdo; o email deve conter apenas o resumo final.
 */
